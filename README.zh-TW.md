...
| **Worker**         | Go (Gin + otelgin)              | `GET /ws`（WebSocket）、`POST /notify`（HTTP，供 API/otelresty 呼叫）；JetStream + Core Subscribe；經 WebSocket 廣播 **traceparent/tracestate + body + api**。 | `8082`                                 |
| **MongoDB**        | Mongo 7 (replica set rs0)       | 儲存 `messaging.messages`；mongotrace 寫入時注入 `_oteltrace`。dbwatcher 監聽 change stream，需 replica set。                          | `27017`                                |
| **dbwatcher**      | Go + mongo-driver               | 監聽 `messaging.messages` **CRUD**（insert / update / replace / delete）；轉發至 NATS JetStream `messages.db`（insert/update/replace 為文件 text，delete 為 `{"op":"delete","id":"..."}`）。Worker 訂閱後經 WebSocket 廣播。 | 無對外 port                            |
...
3. API 的 `otelgin` 延續同一 trace；**JetStream** 路徑使用 `jetstreamtrace.Publish`；**Core** 使用 `natstrace.Publish`；**MongoDB** 路徑使用 mongotrace Collection（Insert/Update/Read/Delete），回傳 `trace_id`。
4. **MongoDB 路徑**：dbwatcher 監聽該 collection 的 change stream，涵蓋 **所有 CRUD**（insert、update、replace、delete）。insert/update/replace 時將文件的 `text` 發布至 NATS JetStream 主題 `messages.db`（有 `_oteltrace` 時一併傳播）；delete 時發布 `{"op":"delete","id":"<hex>"}`。Worker 訂閱後將 **traceparent、tracestate、body、api** 經 WebSocket 廣播。
5. **JetStream / Core 路徑**：Worker 以 JetStream（Consume / Messages）或 Core Subscribe 收訊；`natstrace` / `jetstreamtrace` 從 headers 提取 trace context，建立 receive span，經 WebSocket 廣播。
...
Frontend: send-message-via-worker-http    (SpanKind CLIENT)
  └─ API: POST /api/message-via-worker    (otelgin, SpanKind SERVER)
       └─ API: resty POST /notify         (otelresty, SpanKind CLIENT)
            └─ Worker: POST /notify       (otelgin, SpanKind SERVER)
```
...
- Go 單元測試使用 **testify**（`require` / `assert`），位於 `pkg/natstrace`、`pkg/mongodbtrace`。
- 建議於各模組執行 **`go vet ./...`** 與 **`go test ./...`**（如 `api`、`worker`、`dbwatcher`、`pkg/natstrace`、`pkg/mongodbtrace`）。
...
### Go 服務的 Tracing 初始化與 API（natstrace / mongodbtrace）
...
- **單元測試**：測試中可呼叫 **`natstrace.InitTracer("", natstrace.WithTracerProvider(tp))`** 或 **`mongotrace.InitTracer("", mongotrace.WithTracerProvider(tp))`**（natstrace 若有自訂 propagator 可先 **`otel.SetTextMapPropagator(prop)`**）再 `Connect`/`NewClient`。

---

## Git Submodules

本專案使用下列 submodule（或本地 pkg）：
